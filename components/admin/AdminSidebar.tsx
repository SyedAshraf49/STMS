...
import { DashboardIcon, StudentsIcon, ReportsIcon, LeaveIcon, CloseIcon, FeedbackIcon } from '../icons';
...
const AdminSidebar: React.FC<SidebarProps> = ({ sidebarOpen, setSidebarOpen, currentPage, setCurrentPage, openFeedbackModal }) => {
  const { t } = useI18n();
  ...
  const navItems = [
    ...
    { name: 'Students', page: 'Students', icon: StudentsIcon },
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between p-4 border-b dark:border-gray-700">
          <h1 className="text-base sm:text-xl leading-tight font-bold text-indigo-600 dark:text-indigo-400">{t('sidebar.adminPanel')}</h1>
          <button onClick={() => setSidebarOpen(false)} className="lg:hidden text-gray-600 dark:text-gray-300">
            <CloseIcon className="w-6 h-6"/>
          </button>
        </div>
        <nav className="p-4 flex-grow overflow-y-auto">
          {navItems.map(item => <NavLink key={item.name} name={t(`common.${item.name.toLowerCase()}`)} page={item.page} icon={item.icon} />)}
        </nav>

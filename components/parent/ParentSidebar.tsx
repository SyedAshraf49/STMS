...
import { DashboardIcon, ReportsIcon, LeaveIcon, CloseIcon, FeedbackIcon } from '../icons';
...
const ParentSidebar: React.FC<SidebarProps> = ({ sidebarOpen, setSidebarOpen, currentPage, setCurrentPage, openFeedbackModal }) => {
  const { t } = useI18n();
  ...
  const navItems = [
    { name: t('common.dashboard'), page: 'Dashboard', icon: DashboardIcon },
    { name: t('sidebar.attendanceReport'), page: 'Attendance Report', icon: ReportsIcon },
    { name: t('sidebar.leaveRequests'), page: 'Leave Requests', icon: LeaveIcon },
  ];
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between p-4 border-b dark:border-gray-700">
          <h1 className="text-base sm:text-xl leading-tight font-bold text-indigo-600 dark:text-indigo-400">{t('sidebar.parentPortal')}</h1>
          <button onClick={() => setSidebarOpen(false)} className="lg:hidden text-gray-600 dark:text-gray-300">
            <CloseIcon className="w-6 h-6"/>
          </button>
        </div>
        <nav className="p-4 flex-grow overflow-y-auto">
          {navItems.map(item => <NavLink key={item.page} name={item.name} page={item.page} icon={item.icon} />)}
        </nav>
